...
TEST(anderson_darling, basic) {
	EXPECT_NEAR(anderson_darling_stats({.1, .2})->value, 1.8350, 1e-4); // matches mma
	EXPECT_NEAR(anderson_darling_stats({.1, .9})->value, 0.6186, 1e-4); // matches mma
	EXPECT_NEAR(anderson_darling_stats({.1, .99})->value, 1.6269, 1e-4); // matches mma
	EXPECT_NEAR(anderson_darling_stats({.1, .999})->value, 2.7647, 1e-4); // matches mma
	EXPECT_NEAR(anderson_darling_stats({.1,.2,.3,.4,.5,.6,.7,.8,.9})->value, 0.1549, 1e-4); // mma p-value is 0.1187
}

TEST(anderson_darling, extreme) {
	EXPECT_NEAR(anderson_darling_stats({.1, .9999999})->value, 7.3683, 1e-4);
	EXPECT_NEAR(anderson_darling_stats({.1, 1 - 1e-15})->value, 16.5791, 1e-4); // mma matches (pv=0.0003000)
	EXPECT_NEAR(anderson_darling_stats({.1, 1 - 1e-16})->value, 17.67773, 1e-4); // mma is 0.3093 (pv=0.9431)

	EXPECT_NEAR(anderson_darling_stats({.0000001, .9})->value, 7.3683, 1e-4);
	EXPECT_NEAR(anderson_darling_stats({1e-15, .9})->value, 16.5787, 1e-4); // matches mma
	EXPECT_NEAR(anderson_darling_stats({1e-16, .9})->value, 17.7300, 1e-4); // matches mma

	std::vector<double> vs = {0.87, 0.93, 0.999, 0.99999};
	while (vs.size() < 128) {
		vs.push_back(1);
	}
	EXPECT_NEAR(anderson_darling_stats(vs)->value, 4342.8339, 1e-4); // mma 45.74
	EXPECT_NEAR(anderson_darling_stats(vs)->p_value, 4.6874e-6, 1e-4); // mma 0 -> marsgalias approximation doesn't go further.
}
