...
TEST(avalanche, sac_no_change) {
	...
	EXPECT_EQ(r.size(), 1);
	EXPECT_NEAR(r.front().value,  17.1819, 1e-4);
	EXPECT_NEAR(r.front().p_value, 0.7532, 1e-4);
}

TEST(avalanche, bic_no_change) {
	const auto r = avalanche_mixer_bic_test(50, test_stream(), mx3);
	EXPECT_EQ(r.size(), 1);
	EXPECT_NEAR(r.front().value,  4020.4800, 1e-4);
	EXPECT_NEAR(r.front().p_value, 0.7954, 1e-4);
}
